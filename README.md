...
Having the students open [the included syntax reference notebook](Python%20syntax%20cheat%20sheet.ipynb) can be useful for reinforcing some basics. 

## Class setup
We'll have the latest version of Python 3 installed. We're using [`uv`](https://docs.astral.sh/uv/) to manage the virtual environment and project dependencies (`jupyterlab` and `pandas`), which will already have been installed and tested prior to your session.
...
1. Open Terminal (or `cmd` on a PC)
...
## Run the notebook locally with `uv`
